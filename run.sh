...
$HADOOP_HOME/bin/hadoop fs -put $1/* /user/hadoop/input

# 1. run the first mapreduce task --Estimate
$HADOOP_HOME/bin/hadoop jar $HADOOP_HOME/contrib/streaming/hadoop-streaming-1.0.4.jar \
 -D mapred.job.name="Estimate" \
 -D mapred.reduce.tasks=1 \
 -input /user/hadoop/input \
 -output /user/hadoop/output_Estimate \
...
&& \
$HADOOP_HOME/bin/hadoop fs -get /user/hadoop/output_Estimate/part-* ./ \
&& \
...
$HADOOP_HOME/bin/hadoop jar $HADOOP_HOME/contrib/streaming/hadoop-streaming-1.0.4.jar \
 ...
 -input /user/hadoop/input \
 -output /user/hadoop/output_Materialize \
...
$HADOOP_HOME/bin/hadoop jar $HADOOP_HOME/contrib/streaming/hadoop-streaming-1.0.4.jar \
 ...
 -D mapred.reduce.tasks=1 \
 -input /user/hadoop/output_Materialize/part-* \
...
#$HADOOP_HOME/bin/hadoop fs -get /user/hadoop/output_Estimate ./tmp_hadoop/output_Estimate
#$HADOOP_HOME/bin/hadoop fs -get /user/hadoop/output_Materialize ./tmp_hadoop/output_Materialize
#----------------------------------

# copy the final result files to the local machine
$HADOOP_HOME/bin/hadoop fs -get /user/hadoop/output/part-* $output_path
# remove the intermediate files
$HADOOP_HOME/bin/hadoop fs -rmr /user/hadoop/output_Estimate
$HADOOP_HOME/bin/hadoop fs -rmr /user/hadoop/output_Materialize
rm ./part-*0
